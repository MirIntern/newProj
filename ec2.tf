...
resource "aws_launch_template" "t3microMinikube" {
        ...
        network_interfaces {
         associate_public_ip_address = true
         security_groups = [aws_security_group.allowPort80Ingress.id, aws_security_group.allowPort8080Ingress.id, aws_security_group.minikube.id, aws_security_group.allowAllEgress.id, aws_security_group.sshAccess.id]
        }
        key_name = "LoadBalancerPublic2"
        tags = {
                name = "Server"
